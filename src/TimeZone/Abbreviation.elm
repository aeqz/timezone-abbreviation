...
forZoneName name =
    ...
    case name of
        ...
        "America/Coyhaique" ->
            withError for__america__coyhaique
...
{-| `America/Coyhaique`
-}
for__america__coyhaique : Zone -> Posix -> Result Int Abbreviation
for__america__coyhaique =
    Internal.toTime
        (\time ->
            if time.offsetMinutes == -289 || time.offsetMinutes == -288 then
                Ok Lmt

            else if time.offsetMinutes == -300 || time.offsetMinutes == -240 || time.offsetMinutes == -180 then
                Ok (Offset time.offsetMinutes)

            else if time.offsetMinutes == -283 || time.offsetMinutes == -282 then
                Ok (ShortName "SMT")
